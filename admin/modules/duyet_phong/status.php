...
if ($roomId <= 0 || !in_array($status, ['PENDING','APPROVED','HIDDEN'], true)) {
...
try {
    ...
    $sqlCheck = "
        SELECT r.room_id, r.room_code, r.publish_status,
    ...
    // Chặn duyệt phòng nếu dãy/tòa chưa duyệt
    ...
    // Nếu APPROVED => tạo hóa đơn phí đăng tin nếu cần
    if ($status === 'APPROVED') {
        ...
        // 1) Nếu đã có PAID còn hiệu lực thì không tạo mới
        $sqlPaid = "
        ...
        if (!$paid) {
            // 2) Nếu đã có invoice mở (UNPAID/WAITING_CONFIRM) thì không tạo mới
            $sqlOpen = "
                SELECT svc_invoice_id
                FROM service_invoices
                WHERE room_id = ?
    ...
    // Nếu HIDDEN: hủy invoice mở (tuỳ chọn)
    if ($status === 'HIDDEN') {
