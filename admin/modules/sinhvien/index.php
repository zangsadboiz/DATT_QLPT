...
$countSql = "SELECT COUNT(*) as cnt FROM users u JOIN roles r ON r.role_id = u.role_id WHERE r.role_name = 'STUDENT'";
if ($active === '1' || $active === '0') {
...
// Get booking count for each student
function getBookingCount(mysqli $conn, int $userId): int {
    // Bookings.tenant_id -> tenants.tenant_id, tenants.user_id -> users.user_id
...
<div class="pagetitle">
  <h1><i class="bi bi-person-badge me-2"></i>Quản lý Sinh viên</h1>
  <nav>
    <ol class="breadcrumb">
...
<div class="card">
  <div class="card-header d-flex justify-content-between align-items-center">
    ...
    <a class="btn btn-success btn-sm" href="add.php"><i class="bi bi-plus-circle me-1"></i>Thêm sinh viên</a>
  </div>
  <div class="card-body">
        ...
        <tbody>
          ...
          <?php else: ?>
            <?php foreach ($rows as $i => $u): ?>
              <?php $bookCount = getBookingCount($conn, (int)$u['user_id']); ?>
              <tr>
                <td><?= $i + 1 ?></td>
                ...
                <td>
                  <?php if ($bookCount > 0): ?>
                    <span class="badge bg-info"><?= $bookCount ?> lượt</span>
                  <?php else: ?>
                ...
                <td>
                  <div class="btn-group" role="group">
                    <a class="btn btn-sm btn-outline-info" href="detail.php?id=<?= (int)$u['user_id'] ?>" title="Xem chi tiết"><i class="bi bi-eye"></i></a>
                    <a class="btn btn-sm btn-outline-primary" href="edit.php?id=<?= (int)$u['user_id'] ?>" title="Sửa"><i class="bi bi-pencil"></i></a>
                    <?php if ((int)$u['is_active'] === 1): ?>
                    <a class="btn btn-sm btn-outline-warning" href="toggle.php?id=<?= (int)$u['user_id'] ?>" onclick="return confirm('Khóa tài khoản này?')" title="Khóa"><i class="bi bi-lock"></i></a>
                    <?php else: ?>
                    <a class="btn btn-sm btn-outline-success" href="toggle.php?id=<?= (int)$u['user_id'] ?>" onclick="return confirm('Mở khóa tài khoản này?')" title="Mở khóa"><i class="bi bi-unlock"></i></a>
                    <?php endif; ?>
                  </div>
                </td>
